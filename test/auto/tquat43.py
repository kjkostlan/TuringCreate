...
import numpy as np
from . import tgeom # tgeom.is_all_true(x), tgeom.approx_eq(x1,x2)
...
def matrix_tests(): # Matrixes and vectors.
    ...
    for i in range(16):
        ...
        tests.append(tgeom.approx_eq(points1A,points1B))
        m44A = qmv.m33vTOm44(m33, v)
        tests.append(tgeom.approx_eq(m44A,m44))
...
def quat_tests(): # No matrixes, only quaternions and vectors.
    ...
    for i in range(16):
        ...
        if i==7: # Special cases may cause problems.
            ...
            points[:,1] = [0,0,-1]
        q1_inv = qmv.q_inv(q1)
        tests.append(tgeom.approx_eq(qmv.qq(q1_inv,q1),np.asarray([1,0,0,0])))
        points1 = qmv.qv(q1, points)
        tests.append(tgeom.approx_eq(np.sum(points*points),np.sum(points1*points1)))

        points12A = qmv.qv(q2, qmv.qv(q1, points)) # Test associative.
        points12B = qmv.qv(qmv.qq(q2,q1), points)
        tests.append(tgeom.approx_eq(points12A,points12B))

        axis, radians = qmv.qTOaxisangle(q1) # Test axis angle.
        q1A = qmv.axisangleTOq(axis, radians)
        tests.append(tgeom.approx_eq(q1,q1A) or tgeom.approx_eq(-q1,q1A))
        ...
        tests.append(tgeom.approx_eq(qmv.qv(qp,p0_),p1_))
...
def matrix_quat_tests(): # The BIG ONE.
    ...
    for i in range(16):
        ...
        tests.append(tgeom.approx_eq(points1,points1A))
        ...
        tests.append(tgeom.approx_eq(points2,points2A))
        m44A = qmv.qrvTOm44(q,r,v)
        tests.append(tgeom.approx_eq(m44,m44A))
...
def camera_tests():
    ...
    tests.append(tgeom.approx_eq(v3,v3_green))
    tests.append(tgeom.approx_eq(v3,v3_green1))
    ...
    for i in range(4): # Test the solve 43.
        ...
        tests.append(tgeom.approx_eq(b4_missing1,b4_missing1_green))
    ...
    for i in range(16):
        ...
        tests.append(tgeom.approx_eq(camera44,camera44_green))
        q_, v_, f_, c_, y_, a_ = qmv.cam44TOqvfcya(camera44_green)

        tests.append(tgeom.approx_eq([q, v, f, c, y, a],[q_, v_, f_, c_, y_, a_]))
    ...
    tests.append(tgeom.approx_eq([0,0,-1],center0_green))
    center1_green = qmv.cam44v(cam44,np.expand_dims(v+look_dir*c[1],1))[:,0]
    tests.append(tgeom.approx_eq([0,0,1],center1_green))
    ...
    for i in range(16):
        ...
        tests.append(tgeom.approx_eq(direction,direction_gold))
        tests.append(tgeom.approx_eq(screen_near,[screenxy[0], screenxy[1],-1.0]))
    ...
    tests.append(tgeom.approx_eq([0,0,-1],center_green))
    ...
    tests.append(tgeom.approx_eq([screenx, screeny,-1],onscreen_green))
    ...
    tests.append(tgeom.approx_eq([0,0,-1], qmv.cam44v(cam44,np.expand_dims(v,1))[:,0]))
    tests.append(tgeom.approx_eq([0,0,1], qmv.cam44v(cam44,np.expand_dims(far_center_point,1))[:,0]))
    ...
    tests.append(tgeom.approx_eq(point_in_clip_space, qmv.cam44v(cam44,np.expand_dims(point_in_real_space,1))[:,0]))
