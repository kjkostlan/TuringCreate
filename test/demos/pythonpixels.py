...
import numpy as np
import PythonPixels.texfns as texfns
import PythonPixels.texarray as tnp
from PIL import Image
from matplotlib import pyplot as plt

def debug_image(x, normalize=True, save_file=False, show_window=True, image_name='debug.png'):
...
def simple_veronoi():
    ...
    debug_image(min_dist, image_name='Veronoi')
...
def edge_veronoi():
    ...
    min_dist, second_min_dist, dist_to_edge, index_i, index_j = \
      texfns.veronoi(coords, feature_size=1/32.0, jitter=1.0, pnorm=2.0, h0=128, w0=128,
                     compute_extras=True)
    debug_image(tnp.rgb(dist_to_edge, 0.7*min_dist, second_min_dist), image_name='Veronoi Edges')
...
def index_veronoi():
    ...
    min_dist, second_min_dist, dist_to_edge, index_i, index_j = \
      texfns.veronoi(coords, feature_size=1/scale, jitter=1.0, pnorm=2.0, h0=128, w0=128,
                     compute_extras=True)
    debug_image(tnp.rgb(3*dist_to_edge, np.mod(index_i,3), np.mod(index_j,3)), image_name='Veronoi Index')
...
def hex_veronoi():
    ...
    debug_image(min_dist2*weight+min_dist1*(1.0-weight), image_name='Veronoi Hexagons')
...
def simple_perlin():
    ...
    debug_image(grey, image_name='Perlin noise')
...
def brick_demo():
    ...
    debug_image(tnp.rgb(intense*r,intense,intense*b), image_name='Two brickworks')
    ...
    for mode in range(4):
        if mode==0:
            ...
            grey = texfns.musgrave(coords, steps=12, feature_size0=1/8.0, feature_size1=1/512.0,
                                   ...
                                   h0=128, w0=128)
        #grey = np.abs(-grey)
        debug_image(grey, image_name='Musgrave'+str(mode))
...
def earthquake():
    ...
    debug_image(grey, image_name='Fracture')
...
def marble():
    ...
    green = grey**0.9
    blue = grey**1.0
    rgb = np.ones([grey.shape[0],grey.shape[1],3])
    rgb[:,:,0] = red
    rgb[:,:,1] = green
    rgb[:,:,2] = blue

    debug_image(rgb, image_name='Procedural marble texture')
